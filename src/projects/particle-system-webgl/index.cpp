...
public:
  ...
  float getLength() { return sqrt(pow(x, 2) + pow(y, 2)); }
  ...
  Vector2 *normalize() {
    float normX = x / getLength(), normY = y / getLength();
...
void createParticles(int number, float centerX, float centerY, float radius,
                     float forceStrength) {
  deleteParticles();
...
void createHeavyParticles(int number, float canvasWidth, float canvasHeight) {
  ...
  for (int i = 0; i < number; i++) {
    if (number % 2) {
      x = canvasWidth / 2 +
          radius * cos(M_PI / 2 + 2 * M_PI * (i + 0.5) / number);
      y = canvasHeight / 2 +
          radius * sin(M_PI / 2 + 2 * M_PI * (i + 0.5) / number);
    } else {
      x = canvasWidth / 2 + radius * cos(M_PI / number + 2 * M_PI * i / number);
      y = canvasHeight / 2 +
          radius * sin(M_PI / number + 2 * M_PI * i / number);
    }
...
void respawn(float centerX, float centerY, float radius, float forceStrength) {
  srand(time(nullptr));
...
void spawnEmpty(float centerX, float centerY, float radius, float width,
                float forceStrength) {
  srand(time(nullptr));
...
void explosion(float centerX, float centerY, float explosionForce) {
  Vector2 *force;
  Vector2 *center = new Vector2(centerX, centerY);
...
void calcPartCoordinates(float canvasWidth, float canvasHeight,
                         float particleSize, int isForceApplied,
                         float forceCenterX, float forceCenterY,
                         float deltaTime, int bounceX, int bounceY, int squared,
                         int from, int to) {
  ...
  for (int i = from; i < to; i++) {
    ...
    if (bounceX == 1) {
      if (((particles[i]->position->x - particleSize / 2) < leftBound &&
           particles[i]->velocity->x < 0) ||
          ((particles[i]->position->x + particleSize / 2) > rightBound &&
           particles[i]->velocity->x > 0)) {
    ...
    if (bounceY == 1) {
      if (((particles[i]->position->y - particleSize / 2) < 0 &&
           particles[i]->velocity->y < 0) ||
          ((particles[i]->position->y + particleSize / 2) > canvasHeight &&
           particles[i]->velocity->y > 0)) {
        particles[i]->velocity->y = -particles[i]->velocity->y;
      }
    }

    float timeScale = deltaTime * 240.0f;
    ...
    float frameFriction = pow(particles[i]->friction, timeScale);
    particles[i]->move(frameFriction);
  };
};
...
emscripten::val calcCoordinates(float canvasWidth, float canvasHeight,
                                float particleSize, bool isForceApplied,
                                float forceCenterX, float forceCenterY,
                                float deltaTime, bool bounceX, bool bounceY,
                                bool squared) {
  for (int i = 0; i < NUM_OF_THREADS - 1; i++) {
    threads[i] =
        new thread(calcPartCoordinates, canvasWidth, canvasHeight, particleSize,
                   isForceApplied ? 1 : 0, forceCenterX, forceCenterY,
                   deltaTime, bounceX ? 1 : 0, bounceY ? 1 : 0, squared ? 1 : 0,
                   i * particlesNum / NUM_OF_THREADS,
                   (i + 1) * particlesNum / NUM_OF_THREADS);
  }

  calcPartCoordinates(
      canvasWidth, canvasHeight, particleSize, isForceApplied ? 1 : 0,
      forceCenterX, forceCenterY, deltaTime, bounceX ? 1 : 0, bounceY ? 1 : 0,
      squared ? 1 : 0, (NUM_OF_THREADS - 1) * particlesNum / NUM_OF_THREADS,
      particlesNum);
  ...
  return emscripten::val(
      emscripten::typed_memory_view(particlesNum * 2, particlesCoordinates));
}

EMSCRIPTEN_BINDINGS(particle_system) {
  emscripten::function("createParticles", &createParticles);
  emscripten::function("respawn", &respawn);
  emscripten::function("spawnEmpty", &spawnEmpty);
  emscripten::function("stop", &stop);
  emscripten::function("explosion", &explosion);
  emscripten::function("deleteHeavyParticles", &deleteHeavyParticles);
  emscripten::function("createHeavyParticles", &createHeavyParticles);
  emscripten::function("calcCoordinates", &calcCoordinates);
}
